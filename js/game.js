...
// Licensed under MIT License — copyright © YEAR by Dillo
    ...
    */
    ...
    class Puzzle {
      ...
      */
      ...
      constructor(params) {
        ...
        this.container = (typeof params.container == "string") ?
          ...
          params.container;
          console.log(`This container is: ${params.container}`);
      ...
      getContainerSize() {
        ...
        console.log(`ContWidth : ${this.contWidth }`);
        console.log(`ContHeight: ${this.contWidth }`);
      }
      ...
      create() {
        ...
        */
        ...
        this.defineShapes({ coeffDecentr: 0.12, twistf: [twist0, twist1, twist2, twist3][1 - 1] });
      ...
      */
      ...
      scale() {
        ...
        const maxWidth = 0.85 * this.contWidth;
        const maxHeight = 0.85 * this.contHeight;
    ...
    //Highscore Functions
    //Checking for High Score Eligibility
    async function isHighScore(difficultyName, score) {
      const list = loadHighScoresFor(difficultyName);
      return list.length < 5 || score < list[list.length - 1].scoreValue;
    }

    //Loading highscores
    function loadHighScoresFor(difficulty) {
      const all = JSON.parse(localStorage.getItem('highScores') || '{}');
      return all[difficulty] || [];
    }

    function saveHighScoresFor(difficulty, list) {
      const all = JSON.parse(localStorage.getItem('highScores') || '{}');
      all[difficulty] = list.slice(0, 5);
    ...
    const finalScoreEl = document.getElementById('finalTime');
    const highscoreOverlay = document.getElementById('highscoreOverlay');
    const hsSubmitButton = document.getElementById('hsSubmitButton');
    const hsMenuButton = document.getElementById('hsMenuButton');

    let isPaused = false;

    startButton.addEventListener("click", () => {
      events.push({ event: "nbpieces", nbpieces: totalPieces });
      ...
      startButton.textContent = "Timer: 00:00";

      timerInterval = setInterval(() => {
          const elapsed = Date.now() - startTime;
          const minutes = Math.floor(elapsed / 60000);
          const seconds = Math.floor((elapsed % 60000) / 1000);
          const formattedTime = `Timer: ${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
          startButton.textContent = formattedTime;
      }, 1000);
    });

    // if (hsSubmitButton) {
    //   hsSubmitButton.addEventListener('click', () => {
    //     try {
    //       const difficulty = sessionStorage.getItem('');
    //       const name  = document.getElementById('hsNameInput')
    //                         .value.trim() || 'Anonymous';
    //       const list  = loadHighScoresFor(difficulty);
    //       list.push({ name, score: totalScore });
    //       list.sort((a,b) => b.score - a.score);
    //       saveHighScoresFor(difficulty, list);
    ...
    if (hsSubmitButton) {
      hsSubmitButton.addEventListener('click', () => {
        try {
          const difficulty = sessionStorage.getItem('puzzleLevel') || 'Easy';
          const name = document.getElementById('hsNameInput').value.trim() || 'Anonymous';

          // Use the raw score (time in ms)
          const scoreValue = totalGameTime;

          // Format the time as mm:ss.ttt
          const minutes = Math.floor(scoreValue / 60000);
          const seconds = Math.floor((scoreValue % 60000) / 1000);
          const millis = scoreValue % 1000;
          const scoreLabel = `${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}.${String(millis).padStart(3, '0')}`;

          const list = loadHighScoresFor(difficulty);
          list.push({ name, scoreValue, scoreLabel });

          // Sort by scoreValue (lower time is better)
          list.sort((a, b) => a.scoreValue - b.scoreValue);
    ...
      loadFile(); // Reuse the same function
      pauseOverlay.classList.add('hidden');
      isPaused = false;
      pauseButton.disabled = false;
    });
    ...
    restartButton.addEventListener('click', () => {
      // Reload the page with the current URL and parameters
      ...
      animate = function () {
        ...
        if (event && event.event == "resize") {
          ...
          if (state == 15 || state > 60) { // resize initial or final picture
            puzzle.getContainerSize();
            fitImage(tmpImage, puzzle.contWidth * 0.85, puzzle.contHeight * 0.85);
          }
        ...
        switch (state) {
          ...
          case 60: // winning
            ...
            const elapsedTime = Date.now() - gameStartTimer;
            totalGameTime = elapsedTime;
            console.log(`Total time: ${totalGameTime}`)

            
            setTimeout(async () => {
                const difficultyName = level || 'Easy';
                const minutes = Math.floor(elapsedTime/ 60000);
                const seconds = Math.floor((elapsedTime % 60000) / 1000);
                const millis = elapsedTime % 1000;
                const formattedTime = `${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}.${String(millis).padStart(3, '0')}`;
                const score = elapsedTime;

                const eligible = await isHighScore(difficultyName, score);
                if (eligible) {
                  if (finalScoreEl && highscoreOverlay) {
                    finalScoreEl.textContent = formattedTime;
    ...
    //   for (let k = 1; k < menu.items.length; ++k) {
    //     menu.items[k].element.addEventListener("click", () => events.push({ event: "nbpieces", nbpieces: [12, 25, 50, 100, 200][k - 1] }));
    //   }
    //   return menu;
    // })();
